...
array<double, VERTEX_COUNT> tree_values, tree_add, tree_factor, tree_spendings, tree_values_spendings;

void Init() {
    tree_values.fill(0);
    tree_add.fill(0);
    tree_factor.fill(1);
    tree_spendings.fill(0);
    tree_values_spendings.fill(0);
}
...
void PushSpending(size_t v, size_t l, size_t r) {
    for (size_t w = v * 2; w <= v * 2 + 1; ++w) {
        if (w < VERTEX_COUNT) {
            tree_spendings[w] += tree_spendings[v];
            tree_values_spendings[w] += tree_spendings[v] * (r - l) / 2;
        }
    }
    tree_spendings[v] = 0;
}
...
double ComputeSpendings(size_t v, size_t l, size_t r, size_t ql, size_t qr) {
    if (v >= VERTEX_COUNT || qr <= l || r <= ql) {
        return 0;
    }
    PushSpending(v, l, r);
    if (ql <= l && r <= qr) {
        return tree_values_spendings[v];
    }
    return ComputeSpendings(v * 2, l, (l + r) / 2, ql, qr)
        + ComputeSpendings(v * 2 + 1, (l + r) / 2, r, ql, qr);
}
...
void AddSpending(size_t v, size_t l, size_t r, size_t ql, size_t qr, double value) {
    if (v >= VERTEX_COUNT || qr <= l || r <= ql) {
        return;
    }
    PushSpending(v, l, r);
    if (ql <= l && r <= qr) {
        tree_spendings[v] += value;
        tree_values_spendings[v] += value * (r - l);
        return;
    }
    AddSpending(v * 2, l, (l + r) / 2, ql, qr, value);
    AddSpending(v * 2 + 1, (l + r) / 2, r, ql, qr, value);
    tree_values_spendings[v] =
        (v * 2 < VERTEX_COUNT ? tree_values_spendings[v * 2] : 0)
        + (v * 2 + 1 < VERTEX_COUNT ? tree_values_spendings[v * 2 + 1] : 0);
}

void Multiply(size_t v, size_t l, size_t r, size_t ql, size_t qr, int percentage) {
    if (v >= VERTEX_COUNT || qr <= l || r <= ql) {
        return;
    }
    Push(v, l, r);
    double coef = 1.0 - percentage / 100.0;
    if (ql <= l && r <= qr) {
        tree_factor[v] *= coef;
        tree_add[v] *= coef;
        tree_values[v] *= coef;
...
int main() {
    ...
    for (int i = 0; i < q; ++i) {
        ...
        if (query_type == "ComputeIncome") {
            cout << ComputeSum(1, 0, DAY_COUNT_P2, idx_from, idx_to) - ComputeSpendings(1, 0, DAY_COUNT_P2, idx_from, idx_to) << endl;
        }
            ...
            double value;
            cin >> value;
            AddSpending(1, 0, DAY_COUNT_P2, idx_from, idx_to, value / (idx_to - idx_from));
        }
    }
