...
public:
    SummingSegmentTree(size_t size) : root_(Build({ 0, size })), spendings_root_(Build({ 0, size })) {}

    Data ComputeSum(IndexSegment segment) const {
        return this->TraverseWithQuery(root_, segment, ComputeSumVisitor{});
    }

    Data ComputeSpendings(IndexSegment segment) const {
        return this->TraverseWithQuery(spendings_root_, segment, ComputeSumVisitor{});
    }
    ...
    void AddSpending(IndexSegment segment, const BulkOperation& operation) {
        this->TraverseWithQuery(spendings_root_, segment, AddBulkOperationVisitor{ operation });
    }
...
struct ComputeIncomeRequest : ReadRequest<double> {
    ...
    double Process(const BudgetManager& manager) const override {
        return manager.ComputeSum(MakeDateSegment(date_from, date_to)) - manager.ComputeSpendings(MakeDateSegment(date_from, date_to));
    }
...
struct SpendRequest : ModifyRequest {
    ...
    void Process(BudgetManager& manager) const override {
        ...
        manager.AddSpending(date_segment, BulkMoneyAdder{ daily_spending });
    }
...
struct PayTaxRequest : ModifyRequest {
    ...
    void Process(BudgetManager& manager) const override {
        manager.AddBulkOperation(MakeDateSegment(date_from, date_to), BulkTaxApplier{static_cast<double>(1) - percentage / 100.0} );
    }
...
void PrintResponses(const vector<double>& responses, ostream& stream = cout) {
    for (const double response : responses) {
        stream << std::setprecision(25) << std::fixed << response << endl;
    }
}
...
int main() {
    ...
    cout.precision(25);
    const auto requests = ReadRequests(/*fin*/);
